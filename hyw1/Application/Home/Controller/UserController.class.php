...
class UserController extends BaseController {
    ...
    public function editAction()
    {
        ...
        $data['password'] ? $data['password'] = encrypt(md5($data['password'])) : false;
    ...
    public function userOrderInfo()
    {
        ...
        $Order = M('Order')->alias('o')
                           ->field('grab_userid,order_type,o.order_id,o.order_status,o.order_sn,o.add_time,o.pinpai,o.cart_type,o.dunwei,o.menjia,o.mj_height,o.chexing,g.factorytime,g.use_hours,g.is_status,o.bydc,o.shuju,o.is_yt,o.tenancy,o.yhours,o.mprice,o.number,o.use_user,o.mobile,o.address,o.invoice_title,g.zm_pic,g.cm_pic,g.czt_pic,g.nb_pic,start_time,end_time')
        ...
        if($Order['order_type'] == 2){
            $grab = explode(',',$Order['grab_userid']);
            if($grab[1]){
                $User_car = M('Users')->where(['user_id'=>$grab[1]])->getField('mobile');
                $this->assign('User_car',$User_car);
            }
            $this->assign('token',$this->user['token']);
            $this->display('userOrderInfo2');
        }else{
            $this->display();
        }
        exit;
    ...
    public function userOrderList($order_status='',$page=1,$rows=6)
    {
        ...
        $sql = "SELECT o.order_id,o.order_sn,o.order_status,o.goods_id,o.add_time,o.pinpai,o.cart_type,o.dunwei,g.zm_pic
    ...
    public function carOrderList($status='',$page=1,$rows = 6)
    {
        ...
        $sql    = "SELECT o.order_id,o.order_sn,o.order_status,o.goods_id,o.pinpai,o.cart_type,o.dunwei,g.zm_pic
                   FROM tp_order AS o 
    ...
    public function carOrderInfo()
    {
        ...
        $OrderInfo = M('OrderInfo')->alias('oi')
                                   ->field('o.order_id,oi.status,o.order_sn,oi.add_time,o.pinpai,o.cart_type,o.dunwei,o.menjia,o.mj_height,o.chexing,o.bydc,o.shuju,o.is_yt,o.tenancy,o.yhours,o.mprice,g.zm_pic,g.cm_pic,g.czt_pic,g.nb_pic,oi.cart_age,oi.use_hours,oi.dcsj,oi.paly_num,o.start_time,o.end_time')
...
// dump($Temp);
    ...
    */
    ...
    public function hiddenOrDisplayCar()
    {
        ...
        $res = M('Goods')->where(['user_id'=>$user_id,'goods_id'=>$goods_id])->save($data);
    ...
    public function carInfo()
    {
        ...
        $OrderInfoWhere['oi.paly_num'] = ['gt',0];
        ...
        $count     = M('OrderInfo')->field('o.order_sn,o.start_time,o.end_time,oi.paly_num,oi.zdlzj')
                                   ...
                                   ->where($OrderInfoWhere)
                                   // ->fetchSql()
                                   ->where(['oi.add_time'=>['BETWEEN',"$start_time,$end_time"]])
                                   ->count();
                                   // echo $count;exit;
        if($count < 1){
            $this->assign('page',['page'=>0,'pages'=>0]);
        ...
        $OrderInfo = M('OrderInfo')->field('o.order_id,o.order_sn,o.start_time,o.end_time,oi.add_time,oi.paly_num,oi.zdlzj')
                                   ->alias('oi')
                                   ->join('tp_order as o on oi.order_id = o.order_id')
                                   ->where($OrderInfoWhere)
                                   ->where(['oi.add_time'=>['BETWEEN',"$start_time,$end_time"]])
    ...
    public function userCarInfo()
    {
        ...
        $start_timed = I('start_time')? strtotime(I('start_time')): strtotime('2017-01-01');       
        $end_timed   = I('end_time')  ? strtotime(I('end_time'). ' 23:59:59') : time();    
        $start_time = date('Y-m-d',$start_timed);
        $end_time   = date('Y-m-d 23:59:59',$end_timed);
        ...
        $count = M('Order')->where($where)->where(['add_time'=>['BETWEEN',"$start_time,$end_time"]])->count();
        ...
        $Order = M('Order')->where($where)->where(['add_time'=>['BETWEEN',"$start_time,$end_time"]])->order('add_time DESC')->limit($num,$rows)->select();

        $this->assign('OrderInfo',$Order);
        $this->assign('pageRows',pageRows($page,$pages,6));
        $this->assign('page',['page'=>$page,'pages'=>$pages]);        
        $this->assign('where',['start_time'=>date('Y-n-j',$start_timed),'end_time'=>date('Y-n-j',$end_timed),'old_time'=>$old_time,'old_time2'=>$old_time2]);
        $this->assign('count',['car_count'=>$order_count['number_count'],'overdue_count'=>$overdue_count,'will_be_overdue_count'=>$will_be_overdue_count,'money_count'=>$money_count,'mprice_count'=>$order_count['mprice_count']]);
        $this->display();
    }
    ...
    public function shareHolders()
    {
        ...
        $end_time   = I('end_time') ? strtotime(I('end_time').' 23:59:59') : strtotime('+1 days');
        $page       = I('page',1);
        $rows       = I('rows',8);
        ...
        $AgentFee = M('AgentFee')->alias('af')
                                 ->field('o.order_id,af.add_time,af.order_sn,g.pinpai,g.dunwei,g.cart_type,g.menjia,g.mj_height,g.is_yt,g.shuju,o.address,u1.nickname,u1.mobile,o.number,o.mprice,o.tenancy,af.reward')
                                 ->join('tp_users as u1 on af.consumers_id = u1.user_id')
                                 ->join('tp_order as o on af.order_id = o.order_id')
                                 ->join('tp_goods as g on o.goods_id = g.goods_id')
                                 ->where($where)
    ...
    public function JoiningTrader()
    {
        ...
        // $user_id    = 2651;
        $start_time = I('start_time') ? strtotime(I('start_time')) : strtotime('2017-1-1');
        $end_time   = I('end_time') ? strtotime(I('end_time').' 23:59:59') : strtotime(date('Y-m-d',time()).' 23:59:59');
        $page       = I('page',1);
        $rows       = I('rows',8);
        ...
        $AgentFee = M('AgentFee')->alias('af')
                                 ->field('af.order_id,af.add_time,af.order_sn,g.pinpai,g.dunwei,g.cart_type,g.menjia,g.mj_height,g.is_yt,g.shuju,o.address,u1.nickname,u1.mobile,o.number,o.mprice,o.tenancy,af.reward')
                                 ->join('tp_users as u1 on af.consumers_id = u1.user_id')
                                 ->join('tp_order as o on af.order_id = o.order_id')
                                 ->join('tp_goods as g on o.goods_id = g.goods_id')
                                 ->where($where)
    ...
    //车辆状况
    public function carSn()
    {
        $order_id  = I('order_id');
        $user_id   = $this->user['user_id'];

        $OrderInfo = M('orderInfo')->field('oi.*,o.order_sn,o.start_time,o.end_time')
                                   ->alias('oi')
                                   ->join('tp_order as o on oi.order_id=o.order_id')
                                   ->where(['oi.order_id'=>$order_id,'oi.user_id'=>$user_id,'oi.status'=>5])
                                   ->find();

        $CarInfo = M('CarInfo')->where(['user_id'=>$user_id,'order_id'=>$order_id])->select();
    ...
    public function editCarSn()
    {
    	...
    	$OrderInfo = M('OrderInfo')->where(['order_id'=>$order_id,'user_id'=>$this->user['user_id']])->find();
    	...
    	if($start_time){
    		if($start_time < $OrderInfo['add_time']){
    			exit(json_encode(['status'=>-1,'msg'=>'开始时间必须大于抢单时间！']));
    		}
    		if($start_time > $Order['end_time']){
    			exit(json_encode(['status'=>-1,'msg'=>'开始时间必须小于结束时间！']));
    		}    		
    	...
    	if($end_time){
    		if($end_time < $Order['start_time']){
    			exit(json_encode(['status'=>-1,'msg'=>'结束时间必须大于开始时间！']));
    		}
    ...
    //添加车编
    public function addCarSn()
    {
        $data['car_sn']   = I('car_sn');
        $data['shuju_sn'] = I('shuju_sn');
        $data['order_id'] = I('order_id');

        if(!$data['order_id']){
            exit(json_encode(['status'=>-1,'msg'=>'添加失败，没有该订单！']));
        }        

        if(!is_numeric($data['car_sn'])){
            exit(json_encode(['status'=>-1,'msg'=>'叉车编号必须是数字！']));
        }

        if(strpos($data['car_sn'],'.')){
            exit(json_encode(['status'=>-1,'msg'=>'叉车编号必须是整数！']));
        }

        if($data['car_sn'] > 99999 || $data['car_sn'] < 10000){
            exit(json_encode(['status'=>-1,'msg'=>'叉车编号范围10000-99999！']));
        }

        if(!is_numeric($data['shuju_sn'])){
            exit(json_encode(['status'=>-1,'msg'=>'属具编号必须是数字！']));
        }    

        if(strpos($data['shuju_sn'],'.')){
            exit(json_encode(['status'=>-1,'msg'=>'属具编号必须是整数！']));
        }

        if($data['shuju_sn'] > 99999 || $data['shuju_sn'] < 10000){
            exit(json_encode(['status'=>-1,'msg'=>'属具编号范围10000-99999！']));
        }

        $data['user_id'] = $this->user['user_id'];

        //检查是否超过可添加数量
        $OrderInfo = M('OrderInfo')->where(['order_id'=>$data['order_id'],'user_id'=>$data['user_id'],'status'=>5])->find();
        $count = M('CarInfo')->where(['order_id'=>$data['order_id'],'user_id'=>$data['user_id']])->count();
        
        if($count >= $OrderInfo['paly_num']){
            exit(json_encode(['status'=>-1,'msg'=>'本次订单您只能添加'.$OrderInfo['paly_num'].'个车编，请删除一个再添加！']));
        }

        //检查车编是否重复
        $res2 = M('CarInfo')->where(['order_id'=>$data['order_id'],'car_sn'=>$data['car_sn']])->count();

        if($res2 > 0){
            exit(json_encode(['status'=>-1,'msg'=>'叉车编号重复，请换一个编号！']));
        }

        $res = M('CarInfo')->add($data);

        if($res){
            exit(json_encode(['status'=>1,'msg'=>'添加成功！','id'=>$res]));
        }else{
    ...
    //删除车编
    public function delCarSn()
    {   
        $cid = I('cid');

        $count = M('CarInfo')->where(['cid'=>$cid])->count();

        if($count < 1){
            exit(json_encode(['status'=>1,'msg'=>'删除成功！']));
        }

        $RepairLog = M('RepairLog')->where(['cid'=>$cid])->count();

        if($RepairLog > 0){
            exit(json_encode(['status'=>-1,'msg'=>'有维修记录的车编不能删除！']));
        }

        $res = M('CarInfo')->delete($cid);
    ...
    //车主维修记录
    public function carRepairLog()
    {
        $where['rl.order_id'] = I('order_id');
        $where['ci.user_id']  = $this->user['user_id'];

        $RepairLog = M('RepairLog')->alias('rl')
                                   ->field('rl.*,ci.car_sn,ci.shuju_sn')
                                   ->join('tp_car_info as ci on rl.cid=ci.cid')
                                   ->where($where)
                                   ->order('rl.add_time DESC')
                                   ->select();
        $this->assign('model_type',1);
        $this->assign('RepairLog',$RepairLog);
        $this->display();
    }

    //所有的维修记录
    public function allRepairLog()
    {
        $order_id = I('order_id');

        //如果是车主和客户判断是否是其的订单
        if($this->user['level_id'] < 3){
            $Order = M('Order')->where(['order_id'=>$order_id,'user_id'=>$this->user['user_id']])->find();
            if(!$Order){
                $this->headerUserInfo();
            }
        }

        $RepairLog = M('RepairLog')->where(['order_id'=>$order_id])->order('add_time DESC')->select();
    ...
    public function addRepairLog()
    {
        ...
        if(IS_POST){
            ...
            $CarInfo = M('CarInfo')->where(['cid'=>$data['cid']])->find();

            if($CarInfo['order_id'] != $order_id){
                exit(json_encode(['status'=>-1,'msg'=>'该订单没有此车编！']));
            }else{
                $data['car_id'] = $CarInfo['user_id'];//添加车主id
            }
            if(!$data['repair_time']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写维修时间！']));
            }
            if(!$data['repair_time_long']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写维修时长！']));
            }
            if(!$data['instrument_time']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写仪表时间！']));
            }
            if(!$data['repair_man']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写维修人员！']));
            }
            if(!$data['describe']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写故障描述！']));
            }
            if(!$data['content']){
                exit(json_encode(['status'=>-1,'msg'=>'请填写维修内容！']));
            } 
            ...
            if($_FILES){
                ...
                foreach($_FILES as $key => $val){
                    ...
                    if(!in_array($val['type'],$type)){          
                        exit(json_encode(['status'=>-1,'msg'=>'只支持png、jpg、gif类型的图片！']));
                    }
                }
            }

            //定义上传路径
            $path="./Public/Upload/repairlog/";
            $uploadinfo = $this->fileUploadNews($path,$_FILES);
            $uploadinfo['img1']?$data['img1'] = APP_URL . '/Public/Upload/repairlog/' . $uploadinfo['img1']:'';
            $uploadinfo['img2']?$data['img2'] = APP_URL . '/Public/Upload/repairlog/' . $uploadinfo['img2']:'';
            $uploadinfo['img3']?$data['img3'] = APP_URL . '/Public/Upload/repairlog/' . $uploadinfo['img3']:'';
            $uploadinfo['img4']?$data['img4'] = APP_URL . '/Public/Upload/repairlog/' . $uploadinfo['img4']:'';
            
            $data['add_time'] = time();
            $data['user_id']  = $this->user['user_id'];
            $data['car_sn']   = $CarInfo['car_sn'];
            $data['shuju_sn'] = $CarInfo['shuju_sn'];
            $res = M('RepairLog')->add($data);
        ...
        $carInfo = M('carInfo')->where(['order_id'=>$order_id])->select();
    ...
    {
        $rid = I('rid');
        $Log = M('RepairLog')->where(['rid'=>$rid])->find();

        if($Log['user_id'] != $this->user['user_id']){
            exit(json_encode(['status'=>-1,'msg'=>'只有添加者能删除！']));
        }

        $res = M('RepairLog')->delete($rid);
    ...
    //跳转到个人中心首页
    public function headerUserInfo()
    {
        header('location:'.U('Home/User/userInfo'));
        exit;        
    }
