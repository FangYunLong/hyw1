...
class SpecialController extends BaseController {
    ...
    {
        $dunwei = M('CartArt')->order('sort ASC')->where(['parent_id'=>2])->select();
        $Ad = M('Ad')->where(['ad_id'=>77])->find();
        $pinpai = M('CartArt')->order('sort ASC')->where(['parent_id'=>1])->select();
        $cart_type = M('CartArt')->order('sort ASC')->where(['parent_id'=>3])->select();
        $this->assign('ad',$Ad);
