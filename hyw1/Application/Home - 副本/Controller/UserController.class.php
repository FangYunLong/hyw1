...
// dump($_FILES);exit;
        if($_FILES){
            ...
            $file = fileUploadNews1($path1,$_FILES);
            // dump($file);
...
// dump($Temp);
    ...
    {
        $stact_time = $stact_time ? strtotime($stact_time) : strtotime('2015/01/01');
    ...
    {
        $page = I('get.page');
        $stact_time = I('post.stact_time','2017-1-1');
        $end_time   = I('post.end_time',date('Y-n-j',time()));
    ...
    public function allMsgList($no_read='') 
    {
        ...
        $Users = M('Users')->find($user_id);
        $radioMsg = M('Msg')->where(['type'=>2,'public_time'=>['GT',$Users['reg_time']]])->select();//获取所有的广播消息
    ...
    */
    ...
    public function carInfo()
    {
        ...
        $money_count = M('OrderInfo')->alias('oi')->join('tp_pay as p on oi.order_id = p.order_id')->where($moeny_where)->getField('sum(money)');

        I('old_time1')?$OrderInfoWhere['end_time'] = ['LT',$time]:null;
        I('old_time2')?$OrderInfoWhere['end_time'] = ['BETWEEN',"$end_times,$time"]:null;
        ...
        $this->assign('page',['page'=>$page,'pages'=>$pages]);        
        $this->assign('OrderInfo',$OrderInfo);
        $this->assign('where',['start_time'=>date('Y-m-d',$start_time),'end_time'=>date('Y-m-d',$end_time),'old_time1'=>I('old_time1'),'old_time2'=>I('old_time2')]);
        $this->assign('count',['car_count'=>$car_count,'overdue_count'=>$overdue_count,'will_be_overdue_count'=>$will_be_overdue_count,'money_count'=>$money_count]);
        $this->display();
    }
    ...
    public function userCarInfo()
    {
        ...
        I('get.old_time')?$where['end_time'] = ['LT',time()]:null;
        ...
        $time = time();  
        $end_times = $time + 60*60*24*7; 
        $will_be_overdue_count = M('Order')->where($where)->where(['end_time'=>['BETWEEN',"{$time},{$end_times}"]])->getField('sum(number)');
        
        I('get.old_time2')?$where['end_time'] = ['BETWEEN',"$time,$end_times"]:null;
        $count = M('Order')->where($where)->where(['start_time'=>['BETWEEN',"$start_time,$end_time"]])->count();
...
// dump($_SERVER);
        ...
        $money_where['p.pay_type']   = 2;                                       
        $money_where['p.pay_status'] = 2;
        $money_count = M('Order')->alias('o')->join('tp_pay as p on o.order_id = p.order_id')->where($moeny_where)->getField('sum(money)');
...
// dump($Order);
        $this->assign('OrderInfo',$Order);
        $this->assign('pageRows',pageRows($page,$pages,6));
        $this->assign('where',['start_time'=>date('Y-m-d',$start_time),'end_time'=>date('Y-m-d',$end_time),'old_time'=>$old_time,'old_time2'=>$old_time2]);
        $this->assign('page',['page'=>$page,'pages'=>$pages]);        
        $this->assign('count',['car_count'=>$order_count['number_count'],'overdue_count'=>$overdue_count,'will_be_overdue_count'=>$will_be_overdue_count,'money_count'=>$money_count,'mprice_count'=>$order_count['mprice_count']]);
        $this->display();
    }
