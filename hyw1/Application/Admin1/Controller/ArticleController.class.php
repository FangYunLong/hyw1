...
class ArticleController extends BaseController {
    ...
    public function help_list(){
        $Article = M('Article')->where(['cat_id'=>101])->select();
        // dump($Article);
        $this->assign('Article',$Article);
    	$this->display();   
    }
    
    public function addHelpView()
    {
        $a_id = I('article_id');
    ...
    public function addHelp()
    {
        ...
        if($res){
            $this->success("操作成功",U('Admin/Article/help_list'));
        }else{
            $this->error("操作失败");
        }
    }
