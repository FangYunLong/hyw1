...
class SystemController extends BaseController{
	...
	{          
		/*配置列表*/
        $group_list = array('shop_info'=>'网站信息','smtp'=>'银行卡设置');
        // $group_list = array('smtp'=>'银行卡设置');
        // $group_list = array('shop_info'=>'网站信息','basic'=>'抢单设置','smtp'=>'银行卡设置','sms'=>'APP分享设置');
        ...
        $BankAccount = M('BankAccount')->where(['type'=>2])->select();
    ...
    {
        $id = I('post.id');
        $res = M('BankAccount')->where(['id'=>$id])->delete();
        if($res){
            echo 1;
        }else{
            echo 0;
    ...
    public function addBank()
    {
      ...
      if(!in_array($data['bankname'],$bank)){
        exit(json_encode(['status'=>'-1','msg'=>'暂不支持该银行！']));
      }

      if(!$data['cardholder']){
        exit(json_encode(['status'=>'-1','msg'=>'持卡人不能为空！']));
      }
      // dump($data);exit;
