 ...
 */
    ...
    public function login(){
        ...
        //有token就拿token去缓存中获取用户信息
        if (!empty($token)) {
            //token 非空，则用token获取用户id，从而获取用户数据
            $id = $this->S($token);
            $users = M('User')->where(array('user_id'=>$id))->find();
            session('user',$users);
            setcookie('user_id',$users['user_id'],null,'/');
            setcookie('is_distribut',$users['is_distribut'],null,'/');
            $nickname = empty($users['nickname']) ? $users['mobile'] : $users['nickname'];
