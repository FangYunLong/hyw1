    ...
    */
    public function allMsg()
    {
        ...
        if($no_read){
            ...
            $result = array_merge($radioMsgNo,$msg);
            // dump($result);exit;
            $count = count($result); 
    ...
    */
    ...
    public function carOrderInfo()
    {
        ...
        $OrderInfo = M('OrderInfo')->alias('oi')
                                   ->field('o.address,o.user_id,o.mobile,o.use_user,o.order_id,oi.status,o.order_sn,oi.add_time,o.pinpai,o.cart_type,o.dunwei,o.menjia,o.mj_height,o.bydc,o.shuju,o.is_yt,o.tenancy,o.yhours,o.mprice,g.zm_pic,g.cm_pic,g.czt_pic,g.nb_pic,oi.cart_age,oi.use_hours,oi.dcsj,oi.paly_num,o.start_time,o.end_time')
        ...
        //下单人的身份
        $Users = M('Users')->where(['user_id'=>$OrderInfo['user_id']])->find();
        $OrderInfo['level_id'] = $Users['level_id'];
        if($OrderInfo){
    ...
    public function carOrder()
    {
        ...
        $order_id = M('OrderInfo')->field('order_id,user_id,status,FROM_UNIXTIME(add_time,"%Y-%m-%d %H:%i:%s") as add_time')
                                  ->where($data)
                                  ->limit($num,$rows)
                                  ->order('add_time DESC')
                                  ->select();  
// dump($order_id);
        ...
        $sql    = "SELECT o.order_id,o.order_sn,o.order_status,o.goods_id,o.pinpai,o.cart_type,o.dunwei,g.zm_pic
                   ...
                   ORDER BY field(order_id,{$order_id_str})
                   ";
        $Orders = M('')->query($sql);
...
// dump($Order);
    ...
    public function carTemp()
    {
        ...
        $data['ti.status']  = $status;   
        $data['ti.user_id'] = $user_id;
        ...
        if(!empty($status)&&$status!=3){
            $data['ti.status'] = array('neq',3);
        }

        $data  = array_filter($data);

        $count = M('TempInfo')->alias('ti')->where($data)
                              ->count();
        if($count<1){
            exit(json_encode(['status'=>2,'msg'=>'没有数据！']));
        }
        ...
        $temp_id = M('TempInfo')->alias('ti')
                                ->field('ti.temp_id,ti.status,from_unixtime(ti.add_time,"%Y-%m-%d %H:%i:%s") as push_time,tp.add_time,tp.mobile,tp.username,tp.dunwei,tp.address')
        ...
        // $Temporarys = M('Temporary')->field('temp_id,mobile,username,dunwei,address,add_time')
        //                             ->where($tempData)
        //                             ->select();
    ...
    public function setUserJpush()
    {
        ...
        $data['jpush_status'] = I('jpush_status');
        $data['user_id']      = $user_id;
        $res = M('Users')->save($data);
