...
class SpecialCartController extends BaseController {
    ...
    */
    ...
    public function specialInfo()
    {
        ...
        if(!$goods_id){
            exit(json_encode(['status'=>-1,'msg'=>'参数异常']));
        }
        $res = M('Goods')->field('is_on_sale,pinpai,cart_type,dunwei,menjia,mj_height,shuju,is_yt,use_hours,dcsj,description,address,factorytime,username,mobile,zm_pic,cm_pic,czt_pic,nb_pic,buy_year,special_price,is_status')
                          ->where(array('goods_id'=>$goods_id,'is_special'=>1))
                          ->find();
    ...
    public function specialCartInfo()
    {
        ...
        $res = M('Goods')->field('is_on_sale,pinpai,dunwei,cart_type,menjia,mj_height,shuju,is_yt,cart_age,use_hours,dcsj,description,special_price,address,factorytime,buy_year,username,mobile,zm_pic,cm_pic,czt_pic,nb_pic')
                         ->where(array('goods_id'=>$goods_id,'is_special'=>1,'_logic'=>'AND'))
                         ->select();
        ...
        //dump($res);
        if (!$res)
            exit(json_encode(array('status'=>1,'msg'=>'特价车详情访问失败','result'=>[],'collect_id'=>'')));
    ...
    public function hiddenOrDisplayCar()
    {
        ...
        if($res){
            json_App(['status'=>1,'msg'=>$msg.'成功','is_on_sale'=>$data['is_on_sale']]);
        }else{
            json_App(['status'=>-1,'msg'=>$msg.'失败']);
        }
    }
