...
class RobController extends BaseController {
   ...
   public function lorderInfo()
    {
        $token = I('token');
        $page = isset($_POST['page']) ? I('page') : 1;//当前页
        $rows = isset($_POST['rows']) ? I('rows') : 8;//每一页显示记录数
        $user_id = S($token);
        // $user_id = 2612;
        if(!$user_id){
        ...
        $data['pinpai'] = I('pinpai');
        $data['dunwei'] = I('dunwei');
        $data['order_type'] = I('order_type');
        $data['cart_type'] = I('cart_type');
        ...
        $basic  = tpCache('basic');
        $grab_number = $basic['places'];
        $data['grab_number'] = ['lt',$grab_number];
        ...
        if ($level_id == 2) {
            ...
            $res   = M('Order')->alias('o')
                               // ->join('tp_goods as g on o.goods_id=g.goods_id')
                               ->field('o.order_id,o.goods_id,o.order_status,o.address,pinpai,dunwei')
                               ->where($data)
                               ->where("UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(o.add_time) < {$order_s}")
                               ->page($page,$rows)
    ...
    public function orderInfo(){
        ...
        // $order_id = '806';
        $res = M('Order')->field('user_id,order_sn,goods_id,tenancy,yhours,mprice,number,shuju')->where(array('order_id'=>$order_id))->find();
        $goods_id = $res['goods_id'];
        $good = M('Goods')->field('pinpai,dunwei,cart_type,menjia,mj_height,bydc,is_yt')->where(array('goods_id'=>$goods_id))->find();
        $good['order_sn'] = $res['order_sn'];
        $good['tenancy'] = $res['tenancy'];
        $good['yhours'] = $res['yhours'];
        $good['mprice'] = $res['mprice'];
        $good['number'] = $res['number'];
        $good['shuju'] = $res['shuju'];
        $good['level_id'] = M('Users')->where(['user_id'=>$res['user_id']])->getField('level_id');
    ...
    public function getOrder()
    {
        ...
        if($OrderInfo){
            exit(json_encode(['status'=>-1,'msg'=>'您已抢过该单，请勿重复操作！']));
        }
        ...
        $grab_number = $basic['places'];
        // json_App(['status'=>-1,'msg'=>$grab_number]);

        //先查询订单是否结束
        if($Order['grab_number']>=$grab_number||$Order['order_status']!=0||$Order['is_completed']!=0){
        ...
        if(flock($fp,LOCK_EX))
        {
            ...
            //再次查询抢单是否结束
            if($OrderAgain['grab_number']>=$grab_number||$OrderAgain['order_status']!=0||$OrderAgain['is_completed']!=0){
                $OrderInfoData['status'] = 0;
            }else{
                $OrderInfoData['status'] = 1;
                $OrderData['order_id']    = $OrderAgain['order_id'];
                $OrderData['grab_number'] = $OrderAgain['grab_number'] + 1;
                if($OrderData['grab_number']>=$grab_number){
    ...
    //年月租精简版-车主抢单
    public function pushCarOrderInfo()
    // public function getOrder()
    {
        $token    = I('post.token');
        $user_id  = S($token);
        ...
        // $user_id = 2612;
        // $order_id = 909;
        // $OrderInfoData['order_id'] = $order_id;
        ...
        if($OrderInfoData['status']){
            ...
            $OrderInfoData['add_time'] = time();            
            M('OrderInfo')->add($OrderInfoData);
            $type = ['type'=>7,'order_id'=>$order_id,'order_status'=>5];
            $content = '您的订单已有车主接单，点击查看详情！';
            $content1 = $Order['order_sn'] . '订单已有车主接单，请到个人中心-订单中心查看详情！';            
            Jpush($Order['user_id'],$type,$content,$content1,true);//车主抢单成功，给客户推送消息
            exit(json_encode(['status'=>1,'msg'=>'恭喜，抢单成功！','data'=>$data]));
        }else{
            if(file_exists($path)){
                unlink($path);
            }
    ...
    */
    public function tempList()
    {
        ...
        array_multisort($Temps,SORT_DESC);
    ...
    */
    ...
    public function pushOrder()
    {
        ...
        if($rest){
            ...
            $content1 = $Temp['temp_sn'].'临时租订单已有车主接单，请到个人中心-订单中心查看详情！！';
