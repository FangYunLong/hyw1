...
class MainController extends BaseController {
	...
	*/
    ...
    {
        $res = M('Goods')->field('goods_id,zm_pic,goods_name,bzzj,preferential')->where(array('is_prefer'=>1,'is_on_sale'=>1))->select();
        if(!empty($res)){
	...
	*/
	public function preferInfo()
    {
        ...
        $res = M('Goods')->field('goods_id,goods_name,bzzj,preferential,zm_pic')->where(['goods_id'=>$goods_id])->find();
        if(!empty($res)){
