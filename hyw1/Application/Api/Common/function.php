 ...
 */
function yongyiPays222($user_id='',$orderNo='',$orderAmt='',$type='',$remark='',$true='')
{
 ...
 * @param user_id  用户id
 * @param extras   自定义参数
 * @param content  需要推送的文案
 * @param content1 发送到我的消息的文案
 * @param true     false 则不推送消息，true则进行消息推送
 */
...
function pushLogs($result){  
    ...
    if($result){  
        if(isset($res_arr['error'])){   //如果返回了error则证明失败  
            ...
            $res = "\r\n".date('Y-m-d H:i:s').' success:'.$result;
        }
    }else{      
        //接口调用失败或无响应  
